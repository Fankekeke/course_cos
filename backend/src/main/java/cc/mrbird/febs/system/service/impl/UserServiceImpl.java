...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    @Transactional
    public void regist(String username, String password, Integer type) throws Exception {
        User user = new User();
        user.setPassword(MD5Util.encrypt(username, password));
        ...
        user.setAvatar(User.DEFAULT_AVATAR);
        user.setDescription("滴滴滴");
        user.setUserType(type == 1 ? 2 : 3);
        this.save(user);

        UserRole ur = new UserRole();
        ur.setUserId(user.getUserId());
        if (type == 1) {
            ur.setRoleId(75L); // 教员用户角色 ID
            TeacherInfo teacherInfo = new TeacherInfo();
            teacherInfo.setAdminId(user.getUserId());
            teacherInfo.setCode("T-"+new Date().getTime());
            teacherInfo.setCreateTime(DateUtil.formatDateTime(new Date()));
            teacherInfoService.save(teacherInfo);
        }
        if (type == 2) {
            ur.setRoleId(76L); // 学生用户角色 ID
            StudentInfo studentInfo = new StudentInfo();
            studentInfo.setAdminId(user.getUserId());
            studentInfo.setCode("S-"+new Date().getTime());
            studentInfo.setCreateTime(DateUtil.formatDateTime(new Date()));
