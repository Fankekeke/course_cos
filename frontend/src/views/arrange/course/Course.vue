...
<!--        <a-button @click="batchDelete">删除</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <a-icon v-if="record.adminId === currentUser.userId" type="api" @click="remove(record.id)" title="断 开"></a-icon>
          <a-icon v-if="record.teacherId === null" type="apartment" @click="check(record.id)" title="接 手"></a-icon>
        </template>
      </a-table>
    </div>
    <course-view
...
export default {
  ...
  methods: {
    remove (courseId) {
      this.$get('/cos/course-info/removeCourse', {courseId}).then((r) => {
        this.$message.success('取消课程成功！')
        this.fetch()
      })
    },
    check (courseId) {
      this.$put('/cos/course-info/check', {courseId, userId: this.currentUser.userId}).then((r) => {
        this.$message.success('接手课程成功！')
        this.fetch()
      })
    },
    view (row) {
